...
export function organizeCharms(stash: PlugyStash, items: Item[]) {
  ...
  const classMap = CLASSES.reduce<Record<string, number>>(
    (acc, stat, index) => {
      acc[stat] = index;
      return acc;
    },
  ...
  const charmPropsMap = SORTED_CHARM_PROPS.reduce<Record<string, number>>(
    (acc, stat, index) => {
      acc[stat] = index;
      return acc;
    },
  ...
  items.forEach((item) => {
    const modifiers = item.modifiers?.filter(
      (m) =>
        charmPropsMap[m.stat] !== undefined ||
        m.stat === ALL_SKILL_PROPERTY ||
        m.stat === CLASS_SKILL_PROPERTY
    );
    if (modifiers?.length) {
      const allSkillsModifier = item.modifiers!.find(
      ...
      if (allSkillsModifier) {
        ...
        if (skillsModifier) {
          ...
          const classNameEndIndex = description.indexOf(
            " ",
            parenthesisIndex + 1
          );
          const className = description.substring(
            parenthesisIndex + 1,
            classNameEndIndex
          );
  ...
  const torchGroups = sortAndGroupBy(
    charms.torches,
    (item) => classMap[(item as Torch).torchClassName]
  ) as Torch[][];
  // TODO: Sort torches
  const skillerGroups = sortAndGroupBy(
    charms.skillers,
    (item) => classMap[(item as Skiller).skillerClassName]
  ) as Skiller[][];
  skillerGroups.forEach((items) =>
    items.sort(
      (a, b) =>
        a.skillerDescription.localeCompare(b.skillerDescription) ||
        (b.modifiers?.length || 0) - (a.modifiers?.length || 0)
    )
  );

  const explicitGroups = sortAndGroupBy(charms.explicit, (item) =>
    Math.min(
      ...item
        .modifiers!.map((m) => charmPropsMap[m.stat])
        .filter((m) => m !== undefined)
    )
  );
  explicitGroups.forEach((items) =>
    items.sort(
      (a, b) =>
